...
static esp_err_t firmware_get_handler(httpd_req_t *req)
{
    ...
    snprintf(response, sizeof(response),
             "{\"configured\":%s,\"checking\":%s,\"update_available\":%s,"
             "\"updating\":%s,\"progress_pct\":%d,\"last_error\":\"%s\","
             "\"installed_version\":\"%s\",\"online_version\":\"%s\"}",
             status.configured ? "true" : "false",
             status.checking ? "true" : "false",
             status.update_available ? "true" : "false",
...
static esp_err_t firmware_post_handler(httpd_req_t *req)
{
    ...
    if (httpd_req_get_url_query_len(req) == 0 ||
        ...
        return httpd_resp_send_err(req, HTTPD_400_BAD_REQUEST, "action missing");
    }

    esp_err_t err;
    if (strcmp(action, "check") == 0) {
        err = ota_check_on_connect();
...
esp_err_t api_handle_request(httpd_req_t *req)
{
    ...
    if (req->method == HTTP_GET && uri_path_is(req->uri, "/api/firmware")) {
        return firmware_get_handler(req);
    }
    if (req->method == HTTP_POST && uri_path_is(req->uri, "/api/firmware")) {
        return firmware_post_handler(req);
    }
